...
function single_post(){
	...
	if($content=="No" && strlen($upload_image) >= 1){
		...
		<textarea rows='4' cols='90' placeholder='Write your comment here!!' class='pb-cmnt-textarea' name='comment'></textarea><br>
		<button class='btn btn-info pull-right' name='reply'>finish</button>
       </form>
		</div></div></div>
...
function results(){
	...
	$get_posts="select * from posts where post_content  '%$search_query' OR upload_image like '%$search_query'";
	$run_posts=mysqli_query($con,$get_posts);
	while($row_posts=mysqli_fetch_array($run_posts)){
		$post_id=$row_posts['post_id'];
		$user_id=$row_posts['user_id'];
...
function user_posts(){
	...
	while ($row_posts=mysqli_fetch_array($run_posts)) {
		...
		if ($u_email!=$user_email) {
			echo "<script>window.open('mypost_php?u_id=$user_id','_self')</script>";
		}
		else{
			if($content=="No" && strlen($upload_image) >= 1){
			echo"
...
function search_user(){
	...
	if (isset($_GET['search_user_btn'])) {
		$search_query=htmlentities($_GET['search_users']);
...
<img src='users/$user_image' width='150px' height='140px' id='round' title='$user_name' style='float:left;, margin:1px;'/>
</a>
